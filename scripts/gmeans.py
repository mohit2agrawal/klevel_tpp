...
if __name__ == '__main__':
  filename = sys.argv[1]
  data = np.array([float(x) for x in open(filename).readlines()])
  data = data.reshape((-1, 1))
  print(data.shape)

  data_set = set([x for x in data[:, 0]])
  print(data_set)
  print(len(data_set))
  # exit()
  ...
  print('num of gmean labels:', len(set(gmeans.labels_)))

  # km = MiniBatchKMeans(n_clusters=4)
  # km.fit(data)
  # plot_data['labels_km'] = km.labels_

  # sbn.stripplot(x='x', data=plot_data, hue='labels_gmeans')
  # sbn.stripplot(x='x', data=plot_data, hue='labels_km')

  sbn.stripplot(x='x', data=plot_data, hue='labels_gmeans')

  # sbn.stripplot(x=plot_data['x'], hue=plot_data['labels_gmeans'])
  # sbn.stripplot(x=plot_data['x'], hue=plot_data['labels_km'])
  plt.show()
  # set_trace()
